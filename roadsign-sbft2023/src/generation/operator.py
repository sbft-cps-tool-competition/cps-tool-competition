...
from src.road import Road, random_road, segments_distance


def generate_diverse_roads(rng: np.random.Generator, segments_range, length_range, total_length_range, radius_range,
                           map_size, gen_count, gen_candidates):
    generated = [random_road(rng=rng, segments_range=segments_range, length_range=length_range,
                             total_length_range=total_length_range, radius_range=radius_range, map_size=map_size)]
    for _ in range(gen_count - 1):
        best_candidate = None
        best_diversity = 0.0
        for _ in range(gen_candidates):
            candidate = random_road(rng=rng, segments_range=segments_range, length_range=length_range,
                                    total_length_range=total_length_range, radius_range=radius_range, map_size=map_size)
            diversity = min(segments_distance(candidate.segments, road.segments, length_range=length_range,
                                              radius_range=radius_range) for road in generated)
...
class TestGenerationMutation(Mutation):
    def __init__(self, prob_start=.1, prob_segments=.1, prob_segments_flip=.1, prob_segments_radius=.1,
            ...
            if problem.rng.uniform() < self.prob_segments:
                for segment in road.segments:
                    ...
                    if problem.rng.uniform() < self.prob_segments_radius:
                        segment.radius = problem.rng.uniform(low=problem.radius_range[0],
                                                             high=problem.radius_range[1]) * np.sign(segment.radius)
